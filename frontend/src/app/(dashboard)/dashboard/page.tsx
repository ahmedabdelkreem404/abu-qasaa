...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    { label: dictionary.dashboard.permissions, value: String(user?.permissions.length ?? 0) },
    { label: dictionary.dashboard.reports, value: dictionary.dashboard.live },
  ];
  const roleLabel = user?.roles.map((role) => dictionary.dashboard.roleLabels[role as keyof typeof dictionary.dashboard.roleLabels] ?? role).join(", ")
    || dictionary.dashboard.noRoles;
    ...
    <section className="space-y-6">
      <div className="aq-hero p-6 sm:p-8">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-8 text-white/76">
          {dictionary.dashboard.signedIn} {user?.name}. {dictionary.dashboard.roles}: {roleLabel}.
        </p>
      </div>
          ...
          <div className="mt-3 grid gap-2">
            {user?.business_units.map((unit) => (
              <div key={unit.id} className="rounded-md bg-[var(--aq-soft)] px-3 py-2 text-sm font-bold">
                {locale === "ar" ? unit.name_ar : (unit.name_en ?? unit.name_ar)} · {unit.role}
              </div>
            ))}
          </div>
        )}
